...
def generate_audio(path, rate=200, voice_idx=0):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        for subtitle in subtitles:
            ...
            audio_sum = (
                audio_sum
                + AudioSegment.silent(duration=silence_duration_ms)
                + audio_segment
            )
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        "-p", "--path", help="subtitle file path", default="two_single.srt"
    )
    arg_parser.add_argument(
        "-r", "--rate", help="speech rate(words per minute)", type=int, default=240
    )
    arg_parser.add_argument(
        "-v", "--voice-idx", help="voice selection", type=int, default=1, choices=[0, 1]
    )
